...
func (s *Seeder) createCashbunnyDataForPublicUser(ctx context.Context, tx db.DB, userID uint32, now time.Time) error {
	...
	// Create Category for recurring expense (Groceries)
	cadGroceriesCategoryResult, err := s.querier.CreateTransactionCategory(ctx, tx, queries.CreateTransactionCategoryParams{
		UserID: userID,
		Name:   "Groceries",
	})
	if err != nil {
	...
	cadGroceriesCategoryID, err := cadGroceriesCategoryResult.LastInsertId()
	if err != nil {
		return err
	}
	...
	cadExpenseScheduledTransactionResult, err := s.querier.CreateScheduledTransaction(ctx, tx, queries.CreateScheduledTransactionParams{
		UserID: uint32(userID),
		CategoryID: sql.NullInt32{
			Int32: int32(cadGroceriesCategoryID),
			Valid: true,
		},
		SrcAccountID:  uint32(cadCheckingAccountID),
